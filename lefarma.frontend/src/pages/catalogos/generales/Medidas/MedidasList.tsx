...
import { Ruler, Plus, Pencil, Trash2, Search, Loader2, RefreshCcw, ArrowRightLeft } from 'lucide-react';
...
export default function MedidasList() {
  ...
  const filteredUnidades = unidadesMedida.filter(u =>
      ...
      cell: ({ row }) => {
        ...
        return unidades.length > 0 ? (
          <div className="flex flex-wrap gap-1">
            ...
            {unidades.length > 5 && (
              <Badge variant="secondary" className="text-xs">+{unidades.length - 5}</Badge>
            )}
          </div>
        ) : (
          <span className="text-muted-foreground text-xs">Sin unidades</span>
        );
      },
    },
    {
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex gap-1">
          <Button size="sm" variant="outline" className="h-7" onClick={() => handleEditMedida(row.original.idMedida)}>
            <Pencil className="h-3 w-3" />
          </Button>
          <Button size="sm" variant="destructive" className="h-7" onClick={() => handleDeleteMedida(row.original.idMedida)}>
  ...
  const unidadColumns: ColumnDef<UnidadMedida>[] = [
    ...
    },
    {
      ...
      header: 'Abreviatura',
      cell: ({ row }) => (
        <Badge variant="outline">{row.original.abreviatura}</Badge>
      ),
    },
    {
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex gap-1">
          <Button size="sm" variant="outline" className="h-7 w-7 p-0" onClick={() => handleAbrirReasignar(row.original.idUnidadMedida)} title="Reasignar">
            <ArrowRightLeft className="h-3 w-3" />
          </Button>
          <Button size="sm" variant="outline" className="h-7" onClick={() => handleEditUnidad(row.original)}>
            <Pencil className="h-3 w-3" />
          </Button>
          <Button size="sm" variant="destructive" className="h-7" onClick={() => handleDeleteUnidad(row.original.idUnidadMedida)}>
            <Trash2 className="h-3 w-3" />
          </Button>
        </div>
      ),
  ...
  return (
    <div className="space-y-8">
      ...
      <Modal
        ...
        footer={
          <div className="flex gap-2 justify-end">
            <Button variant="outline" onClick={handleCerrarReasignar}>Cancelar</Button>
            <Button
              disabled={isSavingUnidad}
              onClick={formReasignar.handleSubmit(handleReasignar)}
            >
