...
export const GanttContentHeader: FC<GanttContentHeaderProps> = ({
  ...
  return (
    <div
      className="sticky top-0 z-20 grid w-full shrink-0 bg-muted/80 backdrop-blur-sm border-b border-border/50"
      style={{ height: "var(--gantt-header-height)" }}
    >
      <div>
        <div
          className="sticky inline-flex whitespace-nowrap px-3 py-2 text-foreground font-medium text-xs"
          style={{
      ...
      >
        {Array.from({ length: columns }).map((_, index) => (
          <div
            className="shrink-0 border-border/30 border-b py-1 text-center text-xs text-muted-foreground hover:text-foreground transition-colors"
            key={`${id}-${index}`}
          >
...
export const GanttSidebarItem: FC<GanttSidebarItemProps> = ({
  ...
  const duration = tempEndAt
    ...
    : `${formatDistance(feature.startAt, new Date(), { locale: es })} en curso`;
...
export const GanttSidebarHeader: FC = () => (
  <div
    className="sticky top-0 z-10 flex shrink-0 items-end justify-between gap-2.5 border-border/50 border-b bg-muted/80 p-2.5 font-medium text-foreground text-xs backdrop-blur-sm"
    style={{ height: "var(--gantt-header-height)" }}
  >
    {/* <Checkbox className="shrink-0" /> */}
    <p className="flex-1 truncate text-left">Tareas</p>
    <p className="shrink-0 text-muted-foreground">Duración</p>
  </div>
);
...
export const GanttSidebarGroup: FC<GanttSidebarGroupProps> = ({
  ...
  <div className={cn("bg-card/50", className)}>
    <p
      className="w-full truncate p-2.5 text-left font-semibold text-foreground text-xs border-b border-border/30 bg-muted/40"
    ...
    >
      ...
      >
        ...
        <div className="whitespace-nowrap rounded-full border border-border/50 bg-background/90 px-2 py-1 text-foreground text-xs backdrop-blur-lg">
          {formatDate(date, "dd MMM yyyy", { locale: es })}
        </div>
      </div>
    </div>
  );
...
export const GanttFeatureItemCard: FC<GanttFeatureItemCardProps> = ({
  ...
  return (
    <Card className="h-full w-full rounded-md bg-primary/15 border-primary/30 p-2 text-xs shadow-sm">
      <div
        className={cn(
          "flex h-full w-full items-center justify-between gap-2 text-left text-primary-foreground",
